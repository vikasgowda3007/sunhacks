...
class BookingAgent:
    ...
    def handle_booking(self, state):
        ...
        proficiency = request['sport.value()']

        self.db.save_user_proficiency(user_id, proficiency)
        
        if request['is_Individual']:
            # Individual player looking for a match
            available_matches = self.db.find_match(request['sport.key()'], request['date'], request['time'], proficiency) # sport.key() => sport, sport.value()=> proficiency
            if available_matches:
                ...
                available_courts = self.db.check_availability(request['sport.key()'], request['date'], request['time'])
                if available_courts:
            ...
            if available_courts:
                ...
                state["final_response"] = f"Court for {request['sport.key()']} booked for {request['players']} players on {request['date']} at {request['time']}."
            else:
                state["available_slots"] = []
                state["error"] = "No available courts at the selected time."
                state["final_response"] = "Sorry, no courts are available at that time. Please try a different time."
